...
* Summary
A configuration for kanata that implements the ISRT keyboard layout with homerow modifiers and a custom extend-inspired layout aswell as some other layers and quirks that I use on a daily basis.
